...
month_start = 4 #input the month of study by number
...
if str(Source) == 'Raw':
    ...
    Month_files = str(Data_Source_Folder) + str(date_file_label) + '*_Aeth' + str(file_type)  #+ str(date_file_label)
...
csv_files = glob.glob(Month_files) + glob.glob(Prior_File_1) + glob.glob(Later_File_1) 
...
for csv in csv_files:
    ...
    df = pd.read_csv(csv, skiprows=int(rows_to_skip), header=None, low_memory=False,skip_blank_lines=True, error_bad_lines=True, na_filter=False) ##, usecols=[0,1,5,6,7,8,9,10,11,12]
    frames.append(df)
...
start_Error1 = datetime.datetime(2019,10,30,16,26,00) #next audit
...
plt.plot(aeth_Data['BC Conc (ug/m3)'], label='BC (ug/m3)')
plt.plot(aeth_Data['UVPM_370_nm (ug/m3)'], label='UVPM (ug/m3)')
...
aeth_Data.to_csv(str(AE33_Folder) + 'maqs-AE33-1-' + str(date_file_label) + '-black_carbon-concentration' + str(status) + str(version_number) + '.csv')
